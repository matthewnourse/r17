...
public:
  ...
  void operator()(Input_Stream &input, Output_Stream &output,
    ...
    // Parse the arguments.
    std::vector<std::pair<std::string, rlang::dt::data_type> > arg_pairs = rlang::compiler::eval_to_strings(tokens);
    ...
    std::string file_name_stub = arg_pairs[1].first;
...
private:
  static std::string make_file_name(const std::string &file_name_stub, uint64_t file_counter) {
  ...
  struct async_compress {
    async_compress(const io::heap_buffer_output_stream &current_output_stream, uint64_t file_counter,
                    const std::string &file_name_stub)
    ...
    void operator()() {
      ...
      std::string output_file_name(make_file_name(m_file_name_stub, m_file_counter));
      std::string temp_output_file_name = output_file_name + ".tmp." + uuid::generate().to_string();
      {
