...
public:
  class entry {
  public:
    entry(const std::string &directory_name, const char *file_name)
      : m_directory_name(directory_name), m_file_name(file_name),
        m_relative_path(directory_name + (str::ends_with(directory_name, slash_char()) ? std::string() : slash_char()) + file_name),
    ...
    const std::string &directory_name() const { return m_directory_name; }  
    const std::string &file_name() const { return m_file_name; }
    const std::string &relative_path() const { return m_relative_path; }
  ...
  static bool iterate(const std::string &directory_name, Iterator iter) {
  ...
  template <typename Iterator>
  static void mandatory_iterate(const std::string &directory_name, const Iterator &iter) {
    NP1_ASSERT(iterate(directory_name, iter), "Unable to iterate over directory '" + directory_name + "'");      
  }


  static const std::string &slash_char() {
    static std::string s("/");
    return s;
  }
};
